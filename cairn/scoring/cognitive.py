...
def why_it_might_be_wrong(metrics):
    ...
    return reasons
def detect_curve_bias(curve, metrics):

    biases = []

    if max(curve) / min(curve) > 3:
        biases.append("explosive_growth_unstable")

    if metrics["winrate"] > 0.7 and metrics["max_dd"] < -5:
        biases.append("martingale_like_behavior")

    if len(curve) < 10:
        biases.append("low_sample_curve")
